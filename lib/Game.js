...
class Game {
  ...
  gameLoop() {
    const singlePlayerCondition = this.player1.lives > 0
      && !this.keyboarder.isDown(80)

    const victoryLevel1 = (this.monsterKillCount >= 50
      && this.level === 'level1')
    const victoryLevel2 = (this.level === 'level2' 
      && this.boss.lives === 0 )
    const victoryLevel3 = (this.secretCollected === 10 
      && this.level === 'level3')
  ...
  newGame() {
    ...
    this.player1.x = 175
    this.player1.y = 100
    this.player1.platforms = this.background.platforms
    ...
    setTimeout(() => {
      let spawn = setInterval(() => {
        if (counter >= this.numberEyeSpawn) {
          ...
          this.ctx, this.player1, this.player1));
        counter++;
      }, spawnInterval);
    }, delaySpawn);
