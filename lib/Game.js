...
class Game {
  constructor(canvas, canvas2, canvas3, ctx, ctx2, ctx3) {
    ...
    this.player1 = new Player1(this.ctx, 175, 100, this.background.platforms, this.keyboarder);
    this.player2 = new Player2(this.ctx, 785, 100, this.background.platforms, this.keyboarder);
    this.Bodies = [this.player1, this.player2]; //used ony here

    this.enemies = [];
    this.allArrows = [];
    //this.gameLoop = this.gameLoop.bind(this);
    //game.startGame ... call a method to start
  }
  
  gameLoop() {
    if (this.player1.lives > 0 && this.player2.lives > 0 && !this.keyboarder.isDown(32)) {
      ...
      this.keyboarder.keyState[32] = false;
        if (this.keyboarder.isDown(78)) { //78 = N
          this.newGame();
        }
      requestAnimationFrame(gameLoop)
  ...
  callPowerup() {
    setTimeout(() => {
      ...
      this.player1.this.powerup = this.powerup;
      this.player2.this.powerup = this.powerup;
  ...
  updatePowerup() {
    ...
    if(this.player1.collectPowerup || this.player2.collectPowerup) {
      this.powerup = undefined;
      this.player1.this.powerup = undefined;
      this.player2.this.powerup = undefined;
